...
let movTitles = []
movies.filter(el => movTitles.push(el.Title))
let sortedMovTitles = movTitles.sort()

reloadMovTitle(sortedMovTitles, promo__interactive_list)

function reloadMovTitle(data, place) {
    let count = 0
    place.innerHTML = ''
    for (let item of data) {
        ...
        delete_btn.onclick = () => {
            sortedMovTitles = sortedMovTitles.filter(el => el !== item)
            reloadMovTitle(sortedMovTitles, promo__interactive_list)
        }
